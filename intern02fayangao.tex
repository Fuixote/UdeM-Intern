...
Good afternoon everyone. My name is Weikang Fu. Today I will present the final report of my internship project: \kw{decision-focused learning for kidney exchange programs}.
...
\vspace{0.35em}
The short answer is: in kidney exchange, DFL is not a magic replacement for the two-stage method. It is better understood as a way to rerank candidate exchange plans.

\tr{I will start by connecting this talk with my previous presentation.}
\end{frame}
...
\vspace{0.35em}
This is the main shift of the project: from a method comparison to a mechanism study. In other words, I want to explain the behavior, not only report the numbers.
...
\tr{Next I will show the main experimental setting behind the rest of the talk.}
\end{frame}
...
This slide shows the main experimental backbone after the first talk.
...
\vspace{0.35em}
There are two regimes here. \kw{Step2b} uses noiseless polynomial labels. As the degree increases, the gap becomes larger. \kw{Step2c} adds multiplicative noise, so the task becomes harder, and the whole curve moves upward.

\vspace{0.35em}
The key point for the rest of the talk is this: degree eight, especially in Step2c, is the stress-test regime. The task is hard enough, and the SPO+ signal is visible. So I use this setting to study the mechanism.

\tr{Now I will explain why kidney exchange behaves differently from a standard shortest-path benchmark.}
\end{frame}
...
Shortest path is a very common benchmark for decision-focused learning. It has a serial structure. If the model makes a wrong choice in one part of the path, the whole path can be redirected. So a ranking error can create a large decision gap.

\vspace{0.35em}
Kidney exchange is different. It is more like a packing problem. A solution is made of several cycles and chains. Sometimes one cycle or one chain can be replaced by another one, while the rest of the plan stays almost the same.

\vspace{0.35em}
This creates the main puzzle. DFL often improves shortest path a lot, but in KEP the average gain is more moderate. My explanation is that KEP has many close substitute plans.
...
\vspace{0.35em}
This is important because it creates a simple ceiling. If the two-stage plan is already within epsilon of the oracle, then no method can improve by more than epsilon on that graph.
...
\vspace{0.35em}
For each trained model and graph, I first solve for the best predicted KEP solution. Then I exclude that solution with a no-good cut. After that, I solve again to get rank 2, the next distinct candidate solution. Finally, I evaluate both rank 1 and rank 2 under the true reward.

\vspace{0.35em}
The question is: after rank 1 is removed, does the quality collapse, or is rank 2 still close to the oracle?

\tr{The answer is the first main finding.}
\end{frame}
...
\vspace{0.35em}
For SPO+, rank 2 is also close. The mean gap is \kw{6.32 percent}, and \kw{47.97 percent} are within 5 percent of the oracle.
...
\tr{I also checked whether this pattern is stable when the feasible set changes.}
\end{frame}
...
\vspace{0.35em}
First, I changed the maximum cycle length from 3 to 4 and 5, while keeping the maximum chain length fixed. The rank-2 gap does not become worse. For two-stage, the rank-2 gap goes from 7.04 percent to 6.93 percent and 6.88 percent. For SPO+, it goes from 6.32 percent to 6.20 percent and 5.61 percent.

\vspace{0.35em}
Second, I changed graph density by adding or removing arcs in selected cases. This does change the feasible landscape, and the exact mechanism becomes graph-dependent. But close alternatives often still appear.

\vspace{0.35em}
So the safe reading is: this is not a simple topology law. It is better understood as a graph-instance and feasible-landscape effect.

\tr{Now I move from average patterns to the mechanism of SPO+.}
\end{frame}
...
The second finding is about how DFL helps. In many cases, SPO+ does not create a completely new kind of solution. Instead, it reranks candidate solutions.

\vspace{0.35em}
Here is the idea. Under the two-stage predicted score, candidate A is ranked first, so two-stage selects A. But a near-oracle candidate B may already be in the two-stage top-20 list, just ranked lower, for example at rank 8.

\vspace{0.35em}
SPO+ changes the learned weights, and then B becomes rank 1. If B is near-oracle, this is successful reranking. If B is not near-oracle, then the same reranking move can hurt.

\vspace{0.35em}
So the mechanism is the same. The difference is the quality of the promoted candidate.

\tr{The next two slides show concrete cases.}
\end{frame}
...
This slide shows one concrete graph, G-392.

\vspace{0.35em}
Please do not focus on every single edge in the figure. The main point is the difference among the three plans. The green plan is the oracle, the red plan is the two-stage solution, and the blue plan is the SPO+ solution.

\vspace{0.35em}
In this seed, two-stage selects a high-gap solution. SPO+ recovers the oracle solution. In the mechanism audit, this is a clean correction case: the good candidate is already in the broader candidate list, but two-stage ranks it too low. SPO+ promotes it to the top.

\tr{The next case is similar, but SPO+ does not exactly match the oracle.}
\end{frame}
...
\vspace{0.35em}
Again, the green plan is the oracle, red is two-stage, and blue is SPO+. Here SPO+ does not exactly recover the oracle. But it moves much closer to it.

\vspace{0.35em}
The gap drops from \kw{0.2923} for two-stage to \kw{0.0120} for SPO+. So this is not only a small numerical change. It is a large decision improvement on this graph.
...
I started from features that are available before prediction: number of vertices and arcs, density, degree distribution, components, cycle counts, and feasible exchange geometry.

\vspace{0.35em}
The result is cautious. Single-feature correlations are weak. I do not find a simple rule like ``dense graphs are good for DFL''.

\vspace{0.35em}
But this does not mean graph structure is useless. With matched controls, selected helpful targets remain high relative to graphs with similar coarse topology, while harmful controls remain low.
...
In the final top-20 audit, I used 160 top20-covered graphs, with 50 seeds per graph. These include selected target graphs and matched controls.
...
\vspace{0.35em}
The risk signal is ambiguity and diversity. The same broad candidate boundary also appears in harmful reranking controls, with harmful A-U-R-O-C around 0.81 to 0.823.
