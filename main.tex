...
\[
w_{\text{true}}
=\max\!\left\{0,\ P_{\mathrm{succ}}(u,c)\,\mathrm{QALY}(u,a_d)\,(1+\xi)\right\},
\qquad
\xi\sim\mathcal{N}(0,\sigma^2),\ \sigma=0.15.
\]
\paragraph{Remark.}
...
For validation and test inference, we use deterministic decisions (i.e., no perturbation, typically $\epsilon_t=0$), so each instance is solved once with the learned predicted weights.

\paragraph{Notation.}
Table~\ref{tab:loss-notation} summarizes the main symbols used in the loss definitions below.

\begin{table}[H]
\centering
\caption{Notation used in the loss-function definitions.}
\label{tab:loss-notation}
\begin{tabular}{@{}p{0.34\linewidth}p{0.58\linewidth}@{}}
\toprule
\textbf{Symbol} & \textbf{Meaning} \\
\midrule
\(\mA\) & Edge set of the current graph instance \\
\(w\in\mathbb{R}^{|\mA|}\) & Generic edge-weight vector used as input to \(\mathcal{S}\), \(F_{\epsilon_t}\), and \(\mathcal{L}_{\mathrm{FY}}\) \\
\(w(\theta)\in\mathbb{R}^{|\mA|}\) & Predicted edge-weight vector produced by model parameters \(\theta\) \\
\(\mathbf w_{\mathrm{true}}\in\mathbb{R}^{|\mA|}\) & Ground-truth edge-weight vector \\
\(\mathcal{Y}\subseteq\{0,1\}^{|\mA|}\) & Feasible set of binary edge-incidence decisions \\
\(\mathcal{S}(\cdot)\) & Black-box KEP solver mapping weights to a feasible decision in \(\mathcal{Y}\) \\
\(y(\theta)=\mathcal{S}(w(\theta))\) & Decision induced by the predicted weights \\
\(y^\star=\mathcal{S}(\mathbf w_{\mathrm{true}})\) & Oracle decision under ground-truth weights \\
\(\epsilon_t\) & Perturbation scale at training epoch \(t\) \\
\(\mathbf z^{(m)}\sim\mathcal{N}(\mathbf 0,I)\) & i.i.d.\ Gaussian perturbation sample, \(m=1,\dots,M\) \\
\(M\) & Number of Monte Carlo perturbation samples \\
\(\Omega\) & Convex regularizer on \(\operatorname{conv}(\mathcal{Y})\) \\
\(F_{\epsilon_t}(w)\) & Perturbed maximum under weights \(w\) and scale \(\epsilon_t\) \\
\(y_{\epsilon_t}(w)\in\operatorname{conv}(\mathcal{Y})\) & Perturbed average decision, equal to \(\nabla_w F_{\epsilon_t}(w)\) \\
\(\bar{y}_{\epsilon_t}(w(\theta))\) & Monte Carlo estimate of \(y_{\epsilon_t}(w(\theta))\) \\
\bottomrule
\end{tabular}
\end{table}
...
For a graph instance with edge set \(\mA\), let \(\hat w_e\) denote the predicted weight of edge \(e\in\mA\), and let \(w_{\mathrm{true},e}\) denote its synthetic ground-truth label. Let
\[
w(\theta)=\bigl(\hat w_e\bigr)_{e\in\mA}\in\mathbb{R}^{|\mA|}
\]
denote the vector of all predicted edge weights produced by a model with parameters \(\theta\), and let
\[
\mathbf w_{\mathrm{true}}=\bigl(w_{\mathrm{true},e}\bigr)_{e\in\mA}\in\mathbb{R}^{|\mA|}
\]
denote the corresponding ground-truth weight vector. Both the MLP and GNN two-stage predictors are trained with the same mean squared error objective:
\[
...
\paragraph{What end-to-end DFL actually optimizes}
Let \(\mathcal{S}(\cdot)\) denote the black-box KEP solver returning the binary edge-incidence vector of the optimal exchange plan under a given weight vector. We write
\[
\mathcal{S}: \mathbb{R}^{|\mA|}\to\mathcal{Y},
\qquad
y(\theta)=\mathcal{S}(w(\theta)).
\]
The ideal end-to-end objective is to maximize the true value of the selected plan under the ground-truth edge weights:
\[
\max_\theta \; \mathbf w_{\mathrm{true}}^\top \mathcal{S}(w(\theta)).
\]
Equivalently, one may minimize the regret relative to the oracle plan
\[
y^\star = \mathcal{S}(\mathbf w_{\mathrm{true}}),
\]
namely
\[
\mathcal{R}(\theta)
= \mathbf w_{\mathrm{true}}^\top y^\star
- \mathbf w_{\mathrm{true}}^\top y(\theta).
\]
This is the quantity we truly care about at test time. However, direct gradient-based optimization is blocked by the Jacobian of the discrete solver:
\[
\nabla_\theta \mathcal{R}(\theta)
= -\,\mathbf w_{\mathrm{true}}^\top
\underbrace{\frac{\partial \mathcal{S}(w(\theta))}{\partial w(\theta)}}_{\displaystyle =\,0\ \text{a.e.}}
\cdot \frac{\partial w(\theta)}{\partial \theta}.
\]
Since \(\mathcal{S}(\cdot)\) is a piecewise-constant combinatorial optimizer, its Jacobian with respect to the input weights vanishes almost everywhere, so the exact regret objective is not directly trainable with standard backpropagation. We therefore train with surrogate losses whose gradients encourage lower downstream regret even though they do not optimize \(\mathcal{R}(\theta)\) exactly.

\paragraph{Fenchel--Young surrogate (FY Loss)}

The fundamental obstacle to optimizing \(\mathcal{R}(\theta)\) directly is that
\(\mathcal{S}(\cdot)\) is piecewise constant, so its Jacobian vanishes almost
everywhere.  The Fenchel--Young approach resolves this by replacing the
discrete decision map with a smoothed counterpart constructed via stochastic
perturbation.

\medskip
\noindent\textbf{Perturbed maximizer.}
For a temperature parameter \(\epsilon_t > 0\), define the \emph{perturbed
maximum}
\[
  F_{\epsilon_t}(w)
  \;=\;
  \mathbb{E}_{\mathbf{z}\sim\mathcal{N}(\mathbf{0},I)}
  \!\left[
    \max_{y\in\mathcal{Y}}\,\langle w + \epsilon_t\,\mathbf{z},\,y\rangle
  \right],
\]
and its gradient, the \emph{perturbed average decision},
\[
  y_{\epsilon_t}(w)
  \;=\;
  \nabla_{w} F_{\epsilon_t}(w)
  \;=\;
  \mathbb{E}_{\mathbf{z}\sim\mathcal{N}(\mathbf{0},I)}
  \!\left[
    \mathcal{S}(w + \epsilon_t\,\mathbf{z})
  \right],
\]
where the exchange of gradient and expectation follows from the
absolute-continuity of \(\mathcal{N}(\mathbf{0},I)\) and Danskin's theorem.
Because \(\mathcal{N}(\mathbf{0},I)\) has positive density everywhere,
\(y_{\epsilon_t}(w)\) is differentiable in \(w\) for all \(\epsilon_t>0\),
and satisfies \(y_{\epsilon_t}(w)\to\mathcal{S}(w)\) as \(\epsilon_t\to 0\).

\medskip
\noindent\textbf{Fenchel--Young loss.}
The smoothed maximum can be written as
\[
  F_{\epsilon_t}(w)
  =
  \max_{\mu\in\operatorname{conv}(\mathcal{Y})}
  \bigl[\langle w,\mu\rangle-\epsilon_t\Omega(\mu)\bigr],
\]
so \(F_{\epsilon_t}\) is the Fenchel conjugate of \(\epsilon_t\Omega\). The
Fenchel--Young inequality gives
\[
  \langle w,\,y^{\star}\rangle
  \;\leq\;
  F_{\epsilon_t}(w) + \epsilon_t\,\Omega(y^{\star}),
\]
with equality if and only if \(y^{\star}=y_{\epsilon_t}(w)\).  We define the
\emph{Fenchel--Young loss} as the slack of this inequality:
\[
  \mathcal{L}_{\mathrm{FY}}(w;\,y^{\star})
  \;=\;
  F_{\epsilon_t}(w) + \epsilon_t\,\Omega(y^{\star})
  - \langle w,\,y^{\star}\rangle
  \;\geq\; 0,
\]
which vanishes if and only if \(y_{\epsilon_t}(w)=y^{\star}\), and
whose gradient is
\[
  \nabla_{w}\,\mathcal{L}_{\mathrm{FY}}(w;\,y^{\star})
  \;=\;
  y_{\epsilon_t}(w) - y^{\star}.
\]
This gradient is informative whenever \(y_{\epsilon_t}(w)\neq y^{\star}\),
resolving the vanishing-gradient problem of \(\mathcal{R}(\theta)\).

\medskip
\noindent\textbf{Monte Carlo approximation and linear proxy.}
Because \(y_{\epsilon_t}(w(\theta))\) requires integrating over all
perturbations, we approximate it by drawing \(M\) i.i.d.\ samples
\(\mathbf{z}^{(m)}\sim\mathcal{N}(\mathbf{0},I)\) and solving the KEP under
each perturbed weight vector:
\[
  y^{(m)}
  \;=\;
  \mathcal{S}\!\bigl(w(\theta)+\epsilon_t\,\mathbf{z}^{(m)}\bigr)
  \;\in\;\mathcal{Y},
  \qquad m=1,\dots,M.
\]
The Monte Carlo estimate
\[
  \bar{y}_{\epsilon_t}(w(\theta))
  \;=\;
  \frac{1}{M}\sum_{m=1}^{M} y^{(m)}
  \;\approx\;
  y_{\epsilon_t}(w(\theta))
\]
is an unbiased estimator of the perturbed average decision by the
independence of the samples.

Computing \(F_{\epsilon_t}(w(\theta))\) exactly is intractable, but since the
term \(\epsilon_t\Omega(y^{\star})\) is constant in \(w(\theta)\), the
gradient of \(\mathcal{L}_{\mathrm{FY}}\) is preserved by the linear proxy
\[
  \widetilde{\mathcal{L}}_{\mathrm{FY}}(\theta)
  \;=\;
  w(\theta)^{\!\top}
  \bigl(\bar{y}_{\epsilon_t}(w(\theta)) - y^{\star}\bigr),
\]
where \(\bar{y}_{\epsilon_t}(w(\theta))\) and \(y^{\star}\) are treated as
constants (stop-gradient) during differentiation with respect to \(\theta\).
The resulting gradient with respect to the predicted weights is
\[
  \frac{\partial\,\widetilde{\mathcal{L}}_{\mathrm{FY}}}{\partial\,w(\theta)}
  \;=\;
  \bar{y}_{\epsilon_t}(w(\theta)) - y^{\star},
\]
which is an unbiased Monte Carlo estimate of
\(\nabla_{w}\mathcal{L}_{\mathrm{FY}}\).  The gradient with respect to
\(\theta\) is then obtained by backpropagating through \(w(\theta)\) via the
chain rule:
\[
  \nabla_{\theta}\,\widetilde{\mathcal{L}}_{\mathrm{FY}}
  \;=\;
  \bigl(\bar{y}_{\epsilon_t}(w(\theta))-y^{\star}\bigr)^{\!\top}
  \frac{\partial\,w(\theta)}{\partial\,\theta}.
\]

\medskip
\noindent\textbf{Surrogate validity.}
Minimizing \(\widetilde{\mathcal{L}}_{\mathrm{FY}}\) drives
\(\bar{y}_{\epsilon_t}(w(\theta))\to y^{\star}\), which as \(\epsilon_t\to 0\)
implies \(\mathcal{S}(w(\theta))\to y^{\star}\) and hence
\(\mathcal{R}(\theta)\to 0\).  The two objectives share the same minimizer
under ideal conditions, but in the misspecified setting studied here—where the
linear predictor cannot exactly represent \(\mathbf w_{\mathrm{true}}\)—their
landscape geometries differ, and their respective optima
\(\theta^{*}_{\mathrm{FY}}\) and \(\theta^{*}_{\mathcal{R}}\) need not
coincide.  This distinction is the theoretical motivation for our three-way
empirical comparison.  The construction follows the differentiable
perturbed-optimizer framework of~\citet{berthet2020learning}.
...
This sanity check asks a simple question: how much of the synthetic label can already be explained by the raw edge utility alone? We therefore compare a utility-only baseline on the same split used by the main experiments.

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Edge-level sanity checks for the current synthetic labels on batch \texttt{2026-03-31\_140543}. Correlation is the Pearson correlation between edge utility and the synthetic ground-truth label.}
...
\small
\setlength{\tabcolsep}{4pt}
\caption{Downstream decision quality on the common 10-graph test split of batch \texttt{2026-03-31\_140543}. ``Achieved'' denotes the average ground-truth objective value obtained by the selected exchange plan, and ``Regret'' is measured against the oracle. Methods are grouped by backbone for easier comparison, and the last row is the sanity-check baseline described in the text.}
\label{tab:label-sanity-b}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Method} & \textbf{Avg Achieved} & \textbf{Avg Regret} \\
\midrule
\multicolumn{3}{@{}l}{\textbf{GNN}} \\
2-stage GNN & 308.1811 & 9.1698 \\
FY-DFL GNN & 307.0828 & 10.2681 \\
SPO-DFL GNN & 307.4284 & 9.9225 \\
SPO-DFL + MSE GNN & 307.6322 & 9.7188 \\
\midrule
\multicolumn{3}{@{}l}{\textbf{MLP}} \\
2-stage MLP & 310.2835 & 7.0675 \\
FY-DFL MLP & 309.8360 & 7.5149 \\
SPO-DFL MLP & 309.9184 & 7.4325 \\
SPO-DFL + MSE MLP & 310.2409 & 7.1101 \\
\midrule
...
The utility--label correlation is already high (\(\approx 0.77\)) on every split, so the label is strongly tied to a single local feature. In Table~\ref{tab:label-sanity-b}, the method named \emph{SPO-DFL} refers to \emph{Smart Predict-and-Optimize}-based decision-focused learning. Unlike a two-stage model, which first fits edge weights by a predictive loss such as MSE and only then runs the optimizer, SPO-DFL trains the predictor against downstream decision quality. Let
\[
y^*(w)=\arg\max_{y\in\mathcal{Y}} w^\top y
\]
denote the optimal exchange plan under true edge weights \(w\). The resulting decision regret of a prediction \(\hat w\) is
\[
R(\hat w,w)=w^\top y^*(w)-w^\top y^*(\hat w).
\]
Directly minimizing this regret is difficult, so the implementation uses the standard SPO\(^+\) surrogate: it solves a loss-augmented optimization problem
\[
\tilde y=\arg\max_{y\in\mathcal{Y}} (2\hat w-w)^\top y,
\]
and updates the model using an equivalent training objective proportional to
\[
L_{\mathrm{SPO}^+}(\hat w,w)\propto 2\hat w^\top\bigl(\tilde y-y^*(w)\bigr).
\]
Intuitively, SPO-DFL does not penalize every prediction error equally; it penalizes prediction errors mainly when they change the selected exchange plan and therefore hurt the final optimization objective. We then compare these learned methods and the utility-only baseline under the same downstream hybrid KEP optimizer.

Takeaway: the current synthetic labels are still too easy from an edge-local prediction viewpoint. Since utility alone is already informative and its downstream performance is not far from the learned models, this benchmark currently favors strong two-stage regression over a difficult decision-focused setting.

\subsection{A Solver-Time Comparison Across Formulations}
We also compare solver-only runtime across formulations under the same batch, checkpoint, and solver limits. The goal here is purely computational: how much does formulation choice change stage-2 runtime?
...
Takeaway: formulation choice matters in practice. In this setup, the hybrid solver is the fastest at 3.64\,s, giving a \(3.04\times\) speedup over CF and a clear runtime advantage over PIEF.
