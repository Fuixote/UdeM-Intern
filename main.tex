...
Let \(\hat{\mathbf w}_\theta\) denote the vector of predicted edge weights produced by a model with parameters \(\theta\), and let \(\mathcal{S}(\cdot)\) denote the black-box KEP solver returning the binary edge-incidence vector of the optimal exchange plan under a given weight vector. The downstream decision induced by the model is therefore
\[
\hat{\mathbf y}_\theta = \mathcal{S}(\hat{\mathbf w}_\theta).
\]
The ideal end-to-end objective is to maximize the true value of the selected plan under the ground-truth edge weights \(\mathbf w_{\mathrm{true}}\):
\[
\max_\theta \; \mathbf w_{\mathrm{true}}^\top \mathcal{S}(\hat{\mathbf w}_\theta).
\]
...
\[
\mathcal{R}(\theta)
= \mathbf w_{\mathrm{true}}^\top \mathbf y^\star
- \mathbf w_{\mathrm{true}}^\top \mathcal{S}(\hat{\mathbf w}_\theta).
\]
This is the quantity we truly care about at test time. However, it is not directly suitable for gradient-based learning because \(\mathcal{S}(\cdot)\) is a discrete combinatorial optimizer: the map from \(\hat{\mathbf w}_\theta\) to the selected plan is piecewise constant, non-smooth, and effectively non-differentiable almost everywhere. We therefore train with surrogate losses whose gradients encourage lower downstream regret even though they do not optimize \(\mathcal{R}(\theta)\) exactly.

\paragraph{Fenchel--Young surrogate (FY Loss)}
For the Fenchel--Young approach, we smooth the decision map by perturbing the predicted weights and averaging the resulting solver outputs:
\[
...
\]
For each perturbation sample, define
\[
\mathbf y^{(m)} = \mathcal{S}(\hat{\mathbf w}+\epsilon_t \mathbf z^{(m)}).
\]
The resulting Monte Carlo Fenchel--Young surrogate used in our implementation is
\[
\mathcal{L}_{\mathrm{FY}}(\theta)
= \frac{1}{M}\sum_{m=1}^{M}
\left\langle
...
\]
where \(\operatorname{sg}(\cdot)\) denotes a stop-gradient operator. Its gradient with respect to the predicted edge weights is
\[
\nabla_{\hat{\mathbf w}} \mathcal{L}_{\mathrm{FY}}
= \bar{\mathbf y}_{\epsilon_t}(\hat{\mathbf w}) - \mathbf y^\star.
\]
Thus, FY training pushes the model so that the perturbed average decision induced by \(\hat{\mathbf w}\) moves closer to the oracle decision under \(\mathbf w_{\mathrm{true}}\). This follows the differentiable perturbed-optimizer framework of~\citet{berthet2020learning}.

\paragraph{SPO\(^+\) surrogate}
The SPO route starts from the same regret objective, but replaces it with a loss-augmented optimization surrogate. Define the loss-augmented decision
\[
\tilde{\mathbf y}
= \arg\max_{\mathbf y\in\mathcal Y} (2\hat{\mathbf w}-\mathbf w_{\mathrm{true}})^\top \mathbf y.
\]
The SPO\(^+\) surrogate used in our implementation is equivalent, up to additive constants independent of \(\theta\), to
\[
\mathcal{L}_{\mathrm{SPO}^+}(\theta)
= 2\,\hat{\mathbf w}^\top \bigl(\tilde{\mathbf y}-\mathbf y^\star\bigr).
\]
Its gradient with respect to \(\hat{\mathbf w}\) is proportional to
\[
\nabla_{\hat{\mathbf w}} \mathcal{L}_{\mathrm{SPO}^+}
\propto \tilde{\mathbf y}-\mathbf y^\star.
\]
So unlike FY, which compares the oracle decision with a perturbed average of model-induced decisions, SPO\(^+\) compares the oracle decision with a loss-augmented adversarial decision. Intuitively, SPO\(^+\) penalizes prediction errors mainly when they are large enough to change the selected exchange plan and thereby hurt the final downstream objective.
