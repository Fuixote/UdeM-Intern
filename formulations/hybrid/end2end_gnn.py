    ...
    with torch.no_grad():
        for idx, data in enumerate(dataset, start=1):
            ...
            y_optimal = get_expected_y(
                data.y.view(-1, 1),
                cycle_candidates_from_data(data),
                data.edge_index,
                node_is_ndd,
...
def save_solutions(model, dataset, sol_dir, device, model_tag="GNN-Hybrid-FY"):
    """
    ...
    with torch.no_grad():
        for data in dataset:
            data = data.to(device)
            candidates  = data.candidates
            cycle_candidates = [candidate for candidate in candidates if candidate["type"] == "cycle"]
...
def evaluate_gnn_regret(model, dataset, device, y_optimal_cache=None, solver_cache=None):
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            cycle_candidates = cycle_candidates_from_data(batch)
...
def build_gnn_model_from_config(config):
...
def load_pretrained_gnn_model(pretrain_path, device):
    ckpt = torch.load(pretrain_path, map_location=device)
    state_dict = ckpt["model_state_dict"] if "model_state_dict" in ckpt else ckpt
    config = ckpt.get("config", {}) if isinstance(ckpt, dict) else {}
    model = build_gnn_model_from_config(config).to(device)
...
# 3. 训练主流程
# ==========================================
def train_dfl(pretrain_path=None, data_dir=None, results_root=None, solutions_root=None, use_solver_cache=True):
    ...
    try:
        ...
        # ---- 初始化 ----
        if PRETRAIN_PATH is not None:
            model, pretrain_config = load_pretrained_gnn_model(PRETRAIN_PATH, DEVICE)
            # Stage-1 GNN 输出范围 ~[0, Y_SCALE]，缩放到合理的 logit 范围
            with torch.no_grad():
                model.mlp[-1].weight.data /= Y_SCALE
                model.mlp[-1].bias.data   /= Y_SCALE
            print(f"Loaded pre-trained GNN weights from: {PRETRAIN_PATH}")
            print(
                "Pre-train config: "
                f"NODE_DIM={pretrain_config.get('NODE_DIM', NODE_FEATURE_DIM)}, "
                f"EDGE_RAW_DIM={pretrain_config.get('EDGE_RAW_DIM', EDGE_RAW_DIM)}, "
                f"HIDDEN_DIM={pretrain_config.get('HIDDEN_DIM', 64)}"
            )
        else:
            ...
            print("Training GNN from scratch with Fenchel-Young loss.")

        optimizer     = optim.Adam(model.parameters(), lr=LR)
        best_val_loss = float('inf')

        epoch0_val = evaluate_gnn_regret(
            model,
            val_dataset,
            DEVICE,
        ...
        epoch0_test = evaluate_gnn_regret(
            model,
            test_dataset,
            DEVICE,
            y_optimal_cache=y_optimal_cache,
        ...
        save_solutions(model, full_dataset, sol_dir, DEVICE, model_tag="GNN-Hybrid-FY")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable_solver_cache", action="store_true",
                        help="Disable cached Gurobi model reuse for hybrid training/validation/test.")
