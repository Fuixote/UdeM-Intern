"""Create presentation plots from the current Step1c result archive."""
...
def write_manifest(out_dir: Path, written: list[Path]):
    lines = [
        "# Step1c Plot Results",
        "",
        "Generated from `remote_results/formal_spoplus_ablation_val2000` by default.",
        "",
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Plot Step1c summary figures.")
    parser.add_argument(
        "--results_root",
        default="surrogate_experiment_results/Step1c/remote_results/formal_spoplus_ablation_val2000",
    )
    parser.add_argument(
        "--out_dir",
        default="surrogate_experiment_results/Step1c/plot_results",
    )
    parser.add_argument(
        "--split_path",
        default=str(DEFAULT_SPLIT_PATH),
        help="Shared Step1b/Step1c split file used for the held-out 400-graph test set.",
    )
