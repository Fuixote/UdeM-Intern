"""Evaluate one Step1c run directory on an unseen processed dataset.

This script is intentionally not a sweep driver. It evaluates exactly one
training run directory, which should contain the three standard Step1c model
...
def write_run_config(path: Path, args, graph_count: int, weights):
    payload = {
        ...
        "script": "surrogate_experiment_results/Step1c/evaluate_unseen_run.py",
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--run_dir",
        required=True,
        help="One Step1c run directory, e.g. .../formal_spoplus_ablation_val2000/train_size=50",
    )
    parser.add_argument(
        "--dataset_dir",
