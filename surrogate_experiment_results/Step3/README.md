...
The fixed object is the graph topology:

```text
G_k = fixed vertices, arcs, arc ordering, cycle/chain candidates, and feasible set
```

The random objects are the full edge-feature matrix \(X\) on that fixed topology
and the label vector \(y\) generated from that \(X\). A `train_seed` therefore
means one independent training-dataset replication for the same topology, not a
new topology draw. For formal confirmation:

```text
for each locked topology G_k:
    train_seed = 1    -> one 500-pair training bank on G_k
    train_seed = 2    -> another independent 500-pair training bank on G_k
    ...
    train_seed = 1000 -> the 1000th independent 500-pair training bank on G_k
```

Nested training sizes must be prefixes of the same bank:
...
Do not regenerate separate 50-, 100-, and 500-sample datasets, because that
breaks the paired training-size comparison.

For one sample, the required formal materialization flow is:

```text
base fixed topology
    -> sample X on the frozen vertex/arc set
    -> generate Step2c y from that sampled X
    -> audit topology, arc order, feasible set, and context/label hashes
    -> save sample
```

The current `materialize_phase_b_step2c_datasets.py` path does not yet do this
full flow. It calls `relabel_payload_step2c(base_payload, label_seed=...)`
directly and therefore varies Step2c label noise while leaving \(X\) fixed. That
setting is a valid fixed-X / relabel-y ablation and screening/debugging tool,
but it is not the advisor-facing full-\((X,y)\)-resampling protocol.
...
Formal full-\((X,y)\)-resampling path now lives in separate scripts so the
existing Phase-B fixed-X / relabel-y pipeline keeps its old semantics:

```text
surrogate_experiment_results/Step3/scripts/fixed_topology_xy_common.py
surrogate_experiment_results/Step3/scripts/sample_fixed_topology_context.py
surrogate_experiment_results/Step3/scripts/sample_fixed_topology_xy.py
surrogate_experiment_results/Step3/scripts/build_fixed_eval_sets.py
surrogate_experiment_results/Step3/scripts/build_nested_train_bank.py
surrogate_experiment_results/Step3/scripts/audit_fixed_topology_xy.py
surrogate_experiment_results/Step3/scripts/train_2stage_fixed_topology.py
surrogate_experiment_results/Step3/scripts/train_spoplus_fixed_topology.py
surrogate_experiment_results/Step3/scripts/evaluate_fixed_topology.py
surrogate_experiment_results/Step3/scripts/run_one_job.py
surrogate_experiment_results/Step3/scripts/run_confirmation.py
surrogate_experiment_results/Step3/configs/context_generator.example.yaml
```

The example context-generator config is intentionally marked
...
label generation:
    fixed topology
    fixed X from the processed base payload
    Step2c polynomial degree 8
    multiplicative eps050 relabel
    no fixed-topology context resampling yet
...
The topology hash is checked after every relabel sample. In this current Phase-B
implementation, only labels are allowed to change. In the formal confirmation
implementation, \(X\) and \(y\) must both change while the structural topology,
arc-order, and feasible-set hashes remain fixed.
...
Unless the formal context generator has already been implemented, this
native-e1500 rerun remains a fixed-X / relabel-y screening run. It should be
used for optimization-protocol stabilization and topology pre-screening, not as
the final advisor-facing full-\((X,y)\)-resampling confirmation.
...
\[
X_{k,i}\in\mathbb R^{|E_k|\times 2},
\qquad
X_{k,i,e}=(u_{k,i,e}, c_{k,i,e}).
\]

For one fixed topology, all samples must keep the same edge index and arc
ordering. They differ only in the values of \(u\), \(c\), and the labels
generated from those values.

In the current JSON payloads there are two cPRA locations that must be kept
consistent:

```text
model feature loader:
    target node patient["cPRA"]

Step2c label generator:
    arc field match["recipient_cpra"]
```

When recipient cPRA is resampled, update the target node once and copy the same
value to every incoming arc for that recipient. Do not sample a separate cPRA
value per incoming arc.
...
The \(b_e\), \(\mu_G\), polynomial transformation, and noise multiplication must
be computed after the complete sampled context \(X\) has been written onto the
fixed graph. Because \(\mu_G\) is graph-level, labels cannot be generated by
processing arcs independently.
...
```python
context_payload = sample_context(
...
For the first formal two-feature probe, use the narrowest compatible generator:

```text
resample:
    recipient-level cPRA
    arc-level utility
...
keep fixed:
    ...
    arc set
    arc ordering
    cycle/chain candidate set
```

Required implementation logic:

```python
def sample_context(topology_template, base_payload, context_seed, config):
    out = deepcopy(base_payload)
    rng = np.random.default_rng(context_seed)
    cpra_by_recipient = {}

    for vertex_id, node in out["data"].items():
        if node["type"] == "Pair":
            vertex_key = str(vertex_id)
            base_cpra = node["patient"]["cPRA"]
            new_cpra = bounded_cpra_perturbation(base_cpra, rng, config)
            node["patient"]["cPRA"] = new_cpra
            cpra_by_recipient[vertex_key] = new_cpra

    for source_id, node in out["data"].items():
        for match in node.get("matches", []):
            target_id = str(match["recipient"])
            match["utility"] = bounded_utility_perturbation(
                match["utility"], rng, config
            )
...
Then generate labels from the sampled context:
...
The next implementation gap is therefore an experiment-local
`sample_fixed_topology_context.py` helper, followed by updating the materializer
from:

```text
base_payload -> relabel_payload_step2c(...)
```

to:

```text
base_payload -> sample fixed-topology X -> relabel Step2c y -> audit -> save
```
...
Each topology is its own fixed-topology experiment. Do not interpret one
training replication as a fresh bundle of different topologies.
...
The test set for topology \(G_k\) must contain only samples generated on the
same fixed topology \(G_k\). It must not be a mixed-topology or cross-topology
test set. Cross-topology generalization is a different experiment and is not
the primary Step3 protocol.

For validation and test sets, `train_seed` must be set to a fixed sentinel value
such as `None` or `0`, or omitted from the seed key by a documented convention.
It must not vary with the training replication seed.
...
This script should call an experiment-local context sampler first:

```text
scripts/sample_fixed_topology_context.py
```

The context sampler owns the fixed-topology \(X\)-resampling contract:

```text
input:
    immutable topology template
    base JSON payload
    context_seed
    context_generator_config

output:
    context_payload with resampled recipient cPRA and arc utility
    structural topology unchanged
    recipient cPRA synchronized between target nodes and incoming arcs
```
...
data_generator:
  ...
  resample_X: true
  resample_y: true
  first_formal_probe_resampled_fields:
    recipient_level:
      - patient.cPRA
    arc_level:
      - utility
...
### 15.4 Sample manifest

One row per generated \((X,y)\) pair:

```text
(topology_id, regime, split_namespace, train_seed, sample_index)
```
...
For the formal full-\((X,y)\)-resampling protocol, explicitly verify:

```text
X differs across seeds
y differs across seeds
x_hash differs across distinct context seeds with high probability
label_hash differs across distinct sample keys with high probability
```

Otherwise the run is only a fixed-X label-noise ablation. In Step2b, lack of
distinct \(X\) means there is no valid resampling randomness at all.
...
### 16.8 Context-label consistency

For every generated sample:

```text
model-visible recipient cPRA == Step2c-visible recipient_cpra
all incoming arcs to the same recipient share the same recipient_cpra
utility values used by the model match utility values used by Step2c
label_hash was computed after context resampling and Step2c relabeling
topology_hash, arc_order_hash, and feasible_set_hash are structural hashes
    and do not change when mutable context fields are resampled
```
...
The most important unresolved item is the exact \(X\)-generation mechanism. For
the first formal two-feature probe, the default should be the narrow context
sampler described above: resample recipient-level cPRA and arc-level utility
only, while keeping topology, compatibility, and feasible structure fixed. It
must be implemented and audited before Step2b is included as a formal multi-seed
regime or before Step2c is reported as full-\((X,y)\)-resampling confirmation.
...
[ ] X generator modifies only allowed context fields on the frozen topology
[ ] recipient cPRA is synchronized between node patient["cPRA"] and incoming arc recipient_cpra
[ ] context_seed, label_noise_seed, x_hash, label_hash, topology_hash, arc_order_hash, feasible_set_hash, and generator_config_hash are recorded per sample
[ ] same sample key regenerates bitwise-identical X and y
[ ] different context seeds produce different x_hash values
[ ] Step2b generates distinct X across seeds
[ ] Step2c context and label noise are reproducible
[ ] train sizes are nested
[ ] D50 and D100 are strict prefixes of D500
