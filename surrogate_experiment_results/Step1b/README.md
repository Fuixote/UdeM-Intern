...
evaluated on the 1000-graph unseen processed dataset. On 2026-05-15,
`train_size=1200` was added to the same archived comparison. The unseen evaluation ran
...
| 1200 | 2stage | val MSE | 500 | 9.8848 | 5.0949 | 0.7344 | 0.00534 | -- | -- |
| 1200 | e2e | val decision gap | 130 | 8.2407 | 4.2645 | 0.7364 | 0.00535 | -0.0019 | [-0.0078, 0.0000] |
| 1200 | e2e | val FY loss | 500 | 11.9129 | 5.8105 | 0.7327 | 0.00531 | 0.0017 | [-0.0305, 0.0390] |
...
- The validation-FY-loss-selected e2e checkpoint is slightly better than 2stage
  for `train_size=200` and `1200`, but both paired bootstrap intervals still
  cross zero.
...
runs were also evaluated on the realistic-synthetic stress-test dataset. On
2026-05-15, the completed `train_size=1200` run was added to the same stress
test:
...
| 1200 | 2stage | val MSE | 500 | 9.8848 | 5.0949 | 12.0882 | 0.06592 | 0.93408 | -- | -- |
| 1200 | e2e | val decision gap | 130 | 8.2407 | 4.2645 | 12.0947 | 0.06596 | 0.93404 | -0.0065 | [-0.0149, 0.0000] |
| 1200 | e2e | val FY loss | 500 | 11.9129 | 5.8105 | 12.1526 | 0.06618 | 0.93382 | -0.0645 | [-0.1619, 0.0289] |

Interpretation:

- Under this shifted realistic-synthetic reward label, the noisy-linear-trained
  2stage model is slightly better than or tied with e2e across the completed
  `train_size=50`, `200`, `600`, and `1200` runs.
...
- The e2e checkpoint selected by validation FY loss is worse than 2stage for
  ...
  `train_size=600` and `1200` the intervals cross zero.
...
The current Step1b result archive can be summarized into 20 presentation-ready
figures. They are generated by:

```bash
MPLCONFIGDIR=/tmp/matplotlib conda run -n KEPs \
  python surrogate_experiment_results/Step1b/plot_result_summary.py
```

Default input archive:

```text
surrogate_experiment_results/Step1b/remote_results/formal_M16_2stage500_e2e500_s10
```
...
02_raw_decision_gap_by_dataset
03_achieved_oracle_ratio_by_dataset
04_paired_improvement_over_2stage
05_selected_parameter_endpoints
06_parameter_trajectories
07_08_09_training_curves_combined
07_2stage_mse_loss_curves
08_e2e_fy_objective_curves
09_e2e_decision_gap_curves
10_e2e_validation_tradeoff
11_heldout400_gap_boxplots
12_unseen1000_gap_boxplots
13_realistic2000_gap_boxplots
heldout400_epoch_diagnostics
unseen1000_epoch_diagnostics
realistic2000_epoch_diagnostics
14_heldout400_reward_calibration
15_unseen1000_reward_calibration
16_realistic2000_reward_calibration
```

The `11_*`, `12_*`, and `13_*` figures are per-graph normalized decision-gap
boxplots. They include all three saved checkpoints:

```text
2stage selected by validation MSE
e2e selected by validation decision gap
e2e selected by validation FY loss
```

These figures use the existing per-graph test CSV files:

```text
metrics/test_per_graph.csv
metrics/unseen_test_per_graph.csv
metrics/realistic_unseen_test_per_graph.csv
```

The boxplots hide individual outlier markers for readability and mark the mean
with a white diamond. They are meant to complement the aggregate line/bar plots
by showing whether a method's average gap is driven by broad distributional
movement or by a small number of hard graphs.

The three `*_epoch_diagnostics` figures combine four epoch-wise e2e curves:
train FY objective, validation FY objective, validation synthetic-label decision
gap, and test synthetic-label decision gap. The first three quantities come
from each run's `metrics/e2e_loss_curve.csv`; the test-gap curve is computed
offline from the saved e2e trajectory and cached under:

```text
surrogate_experiment_results/Step1b/remote_results/formal_M16_2stage500_e2e500_s10/train_size=<n>/metrics/<dataset>_epoch_diagnostics.csv
```

Each cached file currently has 51 rows, corresponding to the formal
`metric_stride=10` evaluation epochs:

```text
epoch = 0, 10, 20, ..., 500
```

The `14_*`, `15_*`, and `16_*` figures are test-set reward calibration plots.
For each test dataset and each training size, the script reloads the three saved
model checkpoints, computes edge-level predicted rewards:

```text
w_hat_e(theta) = theta_1 * utility_e + theta_2 * recipient_cPRA_e
```

and compares them against the synthetic label rewards `w_syn_e`. Edges are
sorted by predicted reward and grouped into quantile bins. Each curve plots:

```text
x-axis: mean predicted reward in the bin
y-axis: mean synthetic label reward in the bin
```

The diagonal reference line is perfect reward calibration. Curves above the
diagonal under-predict synthetic rewards; curves below the diagonal over-predict
them. The cached calibration files are intentionally binned summaries rather
than full edge-level dumps, to keep the result directories small:

```text
metrics/heldout400_reward_calibration.csv
