...
STEP1_ABS_DIR="$ROOT_DIR/$STEP1_DIR"
...
FY_EPSILON="${STEP1_FY_EPSILON:-0.01}"
FY_M="${STEP1_FY_M:-16}"
SEED="${STEP1_SEED:-42}"
...
"$PYTHON_BIN" "$STEP1_DIR/plot_trajectories_2D.py" \
  --data_dir "$DATA_DIR" \
  --mse_traj_path "$STEP1_ABS_DIR/trajectory_mse_with_regret.npy" \
  --fy_traj_path "$STEP1_ABS_DIR/trajectory_fy_with_fy_loss_and_regret.npy" \
  --out_dir "$STEP1_DIR" \
  --n_total "$N_TOTAL" \
  --grid_size "$GRID_SIZE" \
  --seed "$SEED" \
...
"$PYTHON_BIN" "$STEP1_DIR/plot_trajectories_3D.py" \
  --mse_path "$STEP1_ABS_DIR/trajectory_mse_with_regret.npy" \
  --fy_path "$STEP1_ABS_DIR/trajectory_fy_with_fy_loss_and_regret.npy" \
  --out_path "$STEP1_ABS_DIR/trajectory_3d_metrics.png"
...
echo "  $STEP1_DIR/trajectory_3d_metrics.png"
