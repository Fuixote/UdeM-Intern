...
LABEL_MODE_STEP2C_POLYNOMIAL_DEGREE_MULTIPLICATIVE_NOISE = "step2c_polynomial_degree_multiplicative_noise"
...
def get_deterministic_uniform(source_key, low, high):
    digest = hashlib.sha256(source_key.encode("utf-8")).digest()
    seed = int.from_bytes(digest[:8], byteorder="big", signed=False)
    rng = random.Random(seed)
...
def normalize_label_mode(label_mode):
    ...
    if label_mode in {
        ...
        LABEL_MODE_STEP2C_POLYNOMIAL_DEGREE_MULTIPLICATIVE_NOISE,
    }:
        return label_mode
...
def processing_config(label_config=None):
    label_config = label_config or {
        ...
        "step2c_degree": DEFAULT_STEP2C_DEGREE,
        "step2c_kappa": DEFAULT_STEP2C_KAPPA,
        "step2c_delta": DEFAULT_STEP2C_DELTA,
        "step2c_epsilon_bar": DEFAULT_STEP2C_EPSILON_BAR,
    ...
    step2c_degree = int(label_config.get("step2c_degree", DEFAULT_STEP2C_DEGREE))
    step2c_kappa = float(label_config.get("step2c_kappa", DEFAULT_STEP2C_KAPPA))
    step2c_delta = float(label_config.get("step2c_delta", DEFAULT_STEP2C_DELTA))
    step2c_epsilon_bar = float(label_config.get("step2c_epsilon_bar", DEFAULT_STEP2C_EPSILON_BAR))
    label_seed = int(label_config.get("label_seed", DEFAULT_LABEL_SEED))
    config = {
    ...
    elif label_mode == LABEL_MODE_STEP2C_POLYNOMIAL_DEGREE_MULTIPLICATIVE_NOISE:
        config.update(
            {
                "label_formula": (
                    "b_e = utility_weight * (utility / 100) + cpra_weight * recipient_cPRA; "
                    "q_e = b_e / (mu_G + delta); r_e = (q_e + kappa)^degree - kappa^degree; "
                ...
                "clean_linear_utility_weight": clean_utility_weight,
                "clean_linear_cpra_weight": clean_cpra_weight,
                "step2c_degree": step2c_degree,
                "step2c_kappa": step2c_kappa,
                "step2c_delta": step2c_delta,
                "step2c_epsilon_bar": step2c_epsilon_bar,
                "label_seed": label_seed,
                "step2c_noise_mode": "deterministic_uniform_multiplicative_per_edge",
                "step2c_rescale_mode": "graph_mean_to_clean_linear_mean_before_noise",
                "step2c_graph_scale": "mu_G = mean clean-linear label over raw valid matches",
            }
        )
    return config
...
def step2c_polynomial_score(clean_linear_value, clean_linear_mean, label_config):
    delta = float(label_config.get("step2c_delta", DEFAULT_STEP2C_DELTA))
    kappa = float(label_config.get("step2c_kappa", DEFAULT_STEP2C_KAPPA))
    degree = int(label_config.get("step2c_degree", DEFAULT_STEP2C_DEGREE))
...
def compute_step2c_graph_label_context(raw_data, label_config):
    valid_vertices = {
        raw_source_vertex_id(node_id, attributes)
        for node_id, attributes in raw_data.items()
    }
    clean_linear_values = []
    for attributes in raw_data.values():
        for match in attributes.get("matches", []):
            if str(match.get("recipient")) not in valid_vertices:
                continue
            clean_linear_values.append(
                clean_linear_utility_cpra_label(
                    match.get("utility", 0),
                    match.get("recipient_cpra"),
                    label_config,
                )
            )

    clean_linear_mean = statistics.mean(clean_linear_values) if clean_linear_values else 0.0
    polynomial_scores = [
        step2c_polynomial_score(value, clean_linear_mean, label_config)[1]
        for value in clean_linear_values
    ]
    polynomial_score_mean = statistics.mean(polynomial_scores) if polynomial_scores else 0.0
    return {
        "clean_linear_mean": float(clean_linear_mean),
        "polynomial_score_mean": float(polynomial_score_mean),
        "clean_linear_edge_count": len(clean_linear_values),
    }


def compute_label_graph_context(raw_data, label_config):
    label_mode = normalize_label_mode(label_config["label_mode"])
    if label_mode == LABEL_MODE_STEP2C_POLYNOMIAL_DEGREE_MULTIPLICATIVE_NOISE:
        return compute_step2c_graph_label_context(raw_data, label_config)
    return {}


def step2c_polynomial_degree_multiplicative_components(
    latent_clean_linear_label,
    label_config,
    source_key,
    graph_label_context=None,
):
    graph_label_context = graph_label_context or {}
    latent = float(latent_clean_linear_label)
    graph_mean = float(graph_label_context.get("clean_linear_mean", latent))
    polynomial_score_mean = float(graph_label_context.get("polynomial_score_mean", 1.0))
    edge_count = int(graph_label_context.get("clean_linear_edge_count", 0))
    delta = float(label_config.get("step2c_delta", DEFAULT_STEP2C_DELTA))
    degree = int(label_config.get("step2c_degree", DEFAULT_STEP2C_DEGREE))
    kappa = float(label_config.get("step2c_kappa", DEFAULT_STEP2C_KAPPA))
    epsilon_bar = float(label_config.get("step2c_epsilon_bar", DEFAULT_STEP2C_EPSILON_BAR))
    label_seed = int(label_config.get("label_seed", DEFAULT_LABEL_SEED))
    q_value, polynomial_score = step2c_polynomial_score(latent, graph_mean, label_config)
    denominator = polynomial_score_mean + delta
    rescaled = graph_mean * polynomial_score / denominator if denominator != 0.0 else 0.0
    polynomial_label = round(max(0.0, rescaled), 4)
    multiplier = get_deterministic_uniform(
        f"{source_key}|step2c_multiplicative_noise|label_seed={label_seed}",
        1.0 - epsilon_bar,
        1.0 + epsilon_bar,
    )
    noisy_label = round(max(0.0, polynomial_label * multiplier), 4)
    return {
        "ground_truth_label": noisy_label,
        "latent_clean_linear_label": round(latent, 4),
        "step2c_polynomial_label": polynomial_label,
        "step2c_multiplier": round(multiplier, 6),
        "step2c_noisy_polynomial_label": noisy_label,
        "step2c_epsilon_bar": epsilon_bar,
        "step2c_q_value": round(q_value, 6),
        "step2c_polynomial_score": round(polynomial_score, 6),
        "step2c_graph_clean_linear_mean": round(graph_mean, 6),
        "step2c_graph_polynomial_score_mean": round(polynomial_score_mean, 6),
        "step2c_graph_clean_linear_edge_count": edge_count,
        "step2c_degree": degree,
        "step2c_kappa": round(kappa, 6),
        "step2c_delta": delta,
    }
...
def compute_ground_truth_label_fields(
    ...
    if label_mode == LABEL_MODE_STEP2C_POLYNOMIAL_DEGREE_MULTIPLICATIVE_NOISE:
        latent = clean_linear_utility_cpra_label(utility, cpra, label_config)
        return step2c_polynomial_degree_multiplicative_components(
            latent,
            label_config,
            source_key,
            graph_label_context,
...
def build_processed_payload(input_file, label_config):
    ...
    graph_label_context = compute_label_graph_context(raw_data, label_config)
    vertices = {}
        ...
        for match in vertex_data.get("matches", []):
            ...
            if target in vertices and (
                ...
                for diagnostic_key in (
                    ...
                    if diagnostic_key in match:
                        selected_match[diagnostic_key] = match[diagnostic_key]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'step2c_polynomial_degree_multiplicative_noise' applies graph-rescaled polynomial labels "
            "and deterministic uniform multiplicative noise."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--step2c_degree",
        type=int,
        default=DEFAULT_STEP2C_DEGREE,
        help="Polynomial degree d for Step2c graph-rescaled nonlinear labels.",
    )
    parser.add_argument(
        "--step2c_kappa",
        type=float,
        default=DEFAULT_STEP2C_KAPPA,
        help="Positive offset kappa used in Step2c polynomial score.",
    )
    parser.add_argument(
        "--step2c_delta",
        type=float,
        default=DEFAULT_STEP2C_DELTA,
        help="Small denominator stabilizer for Step2c graph-level rescaling.",
    )
    parser.add_argument(
        "--step2c_epsilon_bar",
        type=float,
        default=DEFAULT_STEP2C_EPSILON_BAR,
        help="Uniform multiplicative noise half-width: eta is sampled from [1-epsilon_bar, 1+epsilon_bar].",
    )
    parser.add_argument(
        "--label_seed",
        type=int,
        default=DEFAULT_LABEL_SEED,
        help="Seed included in deterministic Step2c per-edge noise keys.",
    )
    parser.add_argument(
        "--output_as_batch_dir",
        action="store_true",
