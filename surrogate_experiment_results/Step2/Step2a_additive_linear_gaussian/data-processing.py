...
LABEL_MODE_STEP2A_ADDITIVE_LINEAR_GAUSSIAN = "step2a_additive_linear_gaussian"
...
def normalize_label_mode(label_mode):
    ...
    if label_mode in {
        ...
        LABEL_MODE_STEP2A_ADDITIVE_LINEAR_GAUSSIAN,
    }:
        return label_mode
...
def label_config_from_args(args):
    return {
        ...
        "step2a_noise_rho": float(args.step2a_noise_rho),
...
def processing_config(label_config=None):
    label_config = label_config or {
        ...
        "step2a_noise_rho": DEFAULT_STEP2A_NOISE_RHO,
    ...
    step2a_noise_rho = float(label_config.get("step2a_noise_rho", DEFAULT_STEP2A_NOISE_RHO))
    ...
    elif label_mode == LABEL_MODE_STEP2A_ADDITIVE_LINEAR_GAUSSIAN:
        config.update(
            {
                "label_formula": (
                    "max(0, b_e + additive_noise), where b_e = utility_weight * "
                    "(utility / 100) + cpra_weight * recipient_cPRA and "
                    "additive_noise ~ Normal(0, (rho * mu_G)^2)"
                ),
                "clean_linear_utility_weight": clean_utility_weight,
                "clean_linear_cpra_weight": clean_cpra_weight,
                "step2a_noise_rho": step2a_noise_rho,
                "label_seed": label_seed,
                "step2a_noise_mode": "deterministic_additive_gaussian_per_edge",
                "step2a_graph_scale": "mu_G = mean clean-linear label over raw valid matches",
            }
        )
    return config
...
def compute_step2a_graph_label_context(raw_data, label_config):
    valid_vertices = {
...
def compute_label_graph_context(raw_data, label_config):
    label_mode = normalize_label_mode(label_config["label_mode"])
    if label_mode == LABEL_MODE_STEP2A_ADDITIVE_LINEAR_GAUSSIAN:
        return compute_step2a_graph_label_context(raw_data, label_config)
    return {}


def step2a_additive_linear_gaussian_components(
    utility,
    cpra,
    label_config,
    source_key,
    ...
    rho = float(label_config.get("step2a_noise_rho", DEFAULT_STEP2A_NOISE_RHO))
    label_seed = int(label_config.get("label_seed", DEFAULT_LABEL_SEED))
    sigma = max(0.0, rho * graph_mean)
    noise = get_deterministic_epsilon(
        f"{source_key}|step2a_additive_noise|label_seed={label_seed}",
        sigma,
    )
    noisy = round(max(0.0, latent + noise), 4)
    return latent, noise, sigma, graph_mean, edge_count, noisy
...
def compute_ground_truth_label_fields(
    ...
    if label_mode == LABEL_MODE_STEP2A_ADDITIVE_LINEAR_GAUSSIAN:
        latent, noise, sigma, graph_mean, edge_count, noisy = step2a_additive_linear_gaussian_components(
            utility,
            cpra,
            label_config,
            source_key,
        ...
        return {
            "ground_truth_label": noisy,
            "latent_clean_linear_label": latent,
            "label_noise_value": round(noise, 6),
            "step2a_noise_sigma": round(sigma, 6),
            "step2a_graph_clean_linear_mean": round(graph_mean, 6),
            "step2a_graph_clean_linear_edge_count": edge_count,
        }
    raise ValueError(f"Unsupported label mode: {label_mode}")
        ...
        for match in vertex_data.get("matches", []):
            ...
            if target in vertices and (
                ...
                for diagnostic_key in (
                    "label_noise_value",
                    "step2a_noise_sigma",
                    "step2a_graph_clean_linear_mean",
                    "step2a_graph_clean_linear_edge_count",
                ):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'step2a_additive_linear_gaussian' applies deterministic additive Gaussian noise "
            "with graph-level scale rho * mean(clean linear label)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--step2a_noise_rho",
        type=float,
        default=DEFAULT_STEP2A_NOISE_RHO,
        help="Additive Gaussian noise strength rho for Step2a labels.",
    )
    parser.add_argument(
        "--label_seed",
        type=int,
        default=DEFAULT_LABEL_SEED,
        help="Seed included in deterministic synthetic-label noise keys.",
    )
    parser.add_argument(
        "--output_as_batch_dir",
        action="store_true",
