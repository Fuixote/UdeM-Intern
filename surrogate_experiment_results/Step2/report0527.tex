...
\begin{itemize}
  ...
  \item the two-feature linear probe, \(\widehat w_e = \theta_1 u_e + \theta_2 c_e\), where \(u_e\) is normalized utility and \(c_e\) is recipient cPRA;
...
Let \(b_e = 10 u_e + 5 c_e\) denote the clean linear signal, and let \(\mu_G\) denote the graph-level mean of \(b_e\). The Step2 grid contains nine label regimes: one Step2a regime, four Step2b degrees, and four Step2c degrees.
...
\begin{itemize}
  ...
  \[
    ...
    \qquad
    \epsilon_e \sim \mathcal N\bigl(0,(\rho\mu_G)^2\bigr),
    \qquad \rho=0.5.
  \]
  ...
  \]
  \[
    w^{\mathrm{syn}}_e = \max\left\{0,\mu_G\frac{r_e^{(d)}}{\frac{1}{|E_G|}\sum_{e'\in E_G} r_{e'}^{(d)} + \delta}\right\},
    \qquad d\in\{1,2,4,8\},\quad \kappa=3.
  \]
  Degree \(d=1\) is close to the clean-linear case; higher degrees increasingly distort the reward surface while keeping graph-level scale controlled.
  ...
  \]
  where \(m_e^{(d)}\) is the graph-rescaled Step2b polynomial label before multiplicative noise. This is the hardest block in the experiment because it combines nonlinear and noisy misspecification.
\end{itemize}
...
\caption{Hard-graph attribution. The x-axis is graph label-to-clean mean ratio, and the y-axis is per-graph normalized-gap improvement over 2stage. Panels show representative regimes at train size 1200. Colors compare FY and SPO+. The vertical line at ratio 1 marks no graph-level label-scale shift; the horizontal line marks no improvement. The figure asks whether decision-focused gains are associated with graph-level label rescaling or distortion.}
\label{fig:hard-graph-attribution}
\end{figure}

\noindent\textbf{How to read.} Points above the horizontal line are graphs where FY or SPO+ improves over 2stage. If improvements concentrate away from ratio 1, then decision-focused gains are associated with graph-level label distortion rather than uniformly distributed across all graphs.
...
Figures 9--14 are representative diagnostics rather than exhaustive visualizations of every run and every graph. All reported quantities are synthetic-label decision quality under designed synthetic regimes. They should not be interpreted as real clinical outcome regret or as claims about actual transplant outcomes.
