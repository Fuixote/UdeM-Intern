...
The per-graph and trajectory diagnostics are representative rather than exhaustive visualizations of every run and every graph. The current plots summarize one fixed seed/protocol grid; additional seed repetitions and a dedicated paired-improvement CI significance figure would be useful before making stronger claims about method dominance. All reported quantities are synthetic-label decision quality under designed synthetic regimes. They should not be interpreted as real clinical outcome regret or as claims about actual transplant outcomes.
