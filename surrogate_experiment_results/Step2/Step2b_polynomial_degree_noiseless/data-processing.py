...
LABEL_MODE_STEP2B_POLYNOMIAL_DEGREE_NOISELESS = "step2b_polynomial_degree_noiseless"
...
DEFAULT_STEP2B_DEGREE = 1
DEFAULT_STEP2B_KAPPA = 3.0
DEFAULT_STEP2B_DELTA = 1e-12
...
def normalize_label_mode(label_mode):
    ...
    if label_mode in {
        ...
        LABEL_MODE_STEP2B_POLYNOMIAL_DEGREE_NOISELESS,
    }:
        return label_mode
...
def label_config_from_args(args):
    return {
        ...
        "step2b_degree": int(args.step2b_degree),
        "step2b_kappa": float(args.step2b_kappa),
        "step2b_delta": float(args.step2b_delta),
...
def processing_config(label_config=None):
    label_config = label_config or {
        ...
        "step2b_degree": DEFAULT_STEP2B_DEGREE,
        "step2b_kappa": DEFAULT_STEP2B_KAPPA,
        "step2b_delta": DEFAULT_STEP2B_DELTA,
    ...
    step2b_degree = int(label_config.get("step2b_degree", DEFAULT_STEP2B_DEGREE))
    step2b_kappa = float(label_config.get("step2b_kappa", DEFAULT_STEP2B_KAPPA))
    step2b_delta = float(label_config.get("step2b_delta", DEFAULT_STEP2B_DELTA))
    ...
    elif label_mode == LABEL_MODE_STEP2B_POLYNOMIAL_DEGREE_NOISELESS:
        config.update(
            {
                "label_formula": (
                ...
                "clean_linear_utility_weight": clean_utility_weight,
                "clean_linear_cpra_weight": clean_cpra_weight,
                "step2b_degree": step2b_degree,
                "step2b_kappa": step2b_kappa,
                "step2b_delta": step2b_delta,
                "label_seed": label_seed,
                "step2b_noise_mode": "noiseless",
                "step2b_rescale_mode": "graph_mean_to_clean_linear_mean",
                "step2b_graph_scale": "mu_G = mean clean-linear label over raw valid matches",
            }
        )
    return config
...
def step2b_polynomial_score(clean_linear_value, clean_linear_mean, label_config):
    delta = float(label_config.get("step2b_delta", DEFAULT_STEP2B_DELTA))
    kappa = float(label_config.get("step2b_kappa", DEFAULT_STEP2B_KAPPA))
    degree = int(label_config.get("step2b_degree", DEFAULT_STEP2B_DEGREE))
...
def compute_step2b_graph_label_context(raw_data, label_config):
    valid_vertices = {
    ...
    polynomial_scores = [
        step2b_polynomial_score(value, clean_linear_mean, label_config)[1]
...
def compute_label_graph_context(raw_data, label_config):
    label_mode = normalize_label_mode(label_config["label_mode"])
    if label_mode == LABEL_MODE_STEP2B_POLYNOMIAL_DEGREE_NOISELESS:
        return compute_step2b_graph_label_context(raw_data, label_config)
    return {}


def step2b_polynomial_degree_noiseless_components(
    ...
    return {
        ...
        "step2b_graph_polynomial_score_mean": round(polynomial_score_mean, 6),
...
def compute_ground_truth_label_fields(
    ...
    if label_mode == LABEL_MODE_STEP2B_POLYNOMIAL_DEGREE_NOISELESS:
        latent = clean_linear_utility_cpra_label(utility, cpra, label_config)
        return step2b_polynomial_degree_noiseless_components(
            latent,
            label_config,
            graph_label_context,
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'step2b_polynomial_degree_noiseless' applies graph-rescaled polynomial labels."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--step2b_degree",
        type=int,
        default=DEFAULT_STEP2B_DEGREE,
        help="Polynomial degree d for Step2b labels.",
    )
    parser.add_argument(
        "--step2b_kappa",
        type=float,
        default=DEFAULT_STEP2B_KAPPA,
        help="Shift kappa in Step2b polynomial score.",
    )
    parser.add_argument(
        "--step2b_delta",
        type=float,
        default=DEFAULT_STEP2B_DELTA,
        help="Numerical stabilizer delta for Step2b graph-level rescaling.",
    )
    parser.add_argument(
        "--label_seed",
        type=int,
        default=DEFAULT_LABEL_SEED,
        help="Synthetic-label seed recorded in metadata for naming/reproducibility.",
    )
    parser.add_argument(
        "--output_as_batch_dir",
        action="store_true",
