...
Item & Current status \\
\midrule
Training source & Same noisy-linear synthetic benchmark as Report0513. \\
\midrule
Validation override & New 2000-graph validation set used for strict Step1b/Step1c checkpoint selection. \\
\midrule
Held-out test & Original 400-graph test split from the fixed Step1b master split. \\
\midrule
Large unseen test & New large out-of-split noisy-linear evaluation. \\
\midrule
Step1b strict FY archive & Available for \(n=50,200,600\) in the current combined report. \\
\midrule
Step1c SPO+ archive & Available for \(n=50,200,600,1200\). \\
\midrule
Combined output & Contains final combined figures and summary tables. \\
\bottomrule
\end{tabular}}
\end{table}
...
\toprule
Method label in figures & Checkpoint selection rule \\
\midrule
2stage (val MSE) & Lowest validation MSE. \\
\midrule
FY (val gap) & Lowest validation synthetic-label decision gap. Diagnostic / oracle-style selection. \\
\midrule
FY (val FY) & Lowest validation \fy{} surrogate objective. Main FY surrogate-selection checkpoint. \\
\midrule
SPO+ (val gap) & Lowest validation synthetic-label decision gap. Diagnostic / oracle-style selection. \\
\midrule
SPO+ (val SPO+) & Lowest validation \spoplus{} objective. Main SPO+ surrogate-selection checkpoint. \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[H]
\centering
\caption{Step1c validation-SPO+ checkpoint diagnostics on the 2000-graph
validation set. The equality rates are fractions of validation graphs.}
\label{tab:step1c-spoplus-diagnostics}
\resizebox{\textwidth}{!}{%
\begin{tabular}{rrrrrrr}
\toprule
Train size & Selected epoch & \(\|\theta\|\) & Val. SPO+ loss & Val. decision gap
& \(\Pr[y^\star(2\hat w-w)=y^\star(w)]\)
& \(\Pr[y^\star(\hat w)=y^\star(w)]\) \\
\midrule
50 & 480 & 10.0108 & 2.8692 & 0.7652 & 0.2415 & 0.5515 \\
\midrule
200 & 500 & 10.0769 & 2.8017 & 0.7367 & 0.2620 & 0.5555 \\
\midrule
600 & 500 & 10.2428 & 2.7924 & 0.7368 & 0.2700 & 0.5555 \\
\midrule
1200 & 500 & 10.0567 & 2.8007 & 0.7355 & 0.2635 & 0.5555 \\
\bottomrule
\end{tabular}}
\end{table}

\paragraph{How to read Table~\ref{tab:step1c-spoplus-diagnostics}.}
The selected epoch is the checkpoint with the lowest validation \spoplus{}
objective, not the checkpoint with the lowest validation decision gap. The last
two columns should be read together. The prediction-induced solution
\(y^\star(\hat w)\) already matches the synthetic-label oracle on about \(55\%\)
of validation graphs, while the adversarial SPO+ challenger
\(y^\star(2\hat w-w)\) matches the oracle on only about \(24\%-27\%\) of graphs.
This means that many predicted decisions are already correct, but the SPO+
margin condition is still stricter: the shifted adversarial objective can still
find non-oracle challenger solutions on most validation instances.
...
\centering
\small
\caption{Direct comparison of 2stage, FY, and SPO+ on heldout400 and large
unseen tests. Entries are mean normalized decision gap with the selected epoch
in parentheses; lower is better.}
\label{tab:main-pk-step1bc}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llcccccc}
\toprule
Test set & Train size & 2stage (val MSE) & FY (val gap) & FY (val FY) &
SPO+ (val gap) & SPO+ (val SPO+) & Best \\
\midrule
Heldout400 & 50 & 0.006110 (500) & 0.005918 (50) & 0.005467 (440) &
0.005809 (100) & \textbf{0.005442 (480)} & SPO\!+ val SPO\!+ \\
\midrule
 & 200 & 0.006095 (500) & 0.005809 (240) & 0.005629 (500) &
0.005891 (130) & \textbf{0.005610 (500)} & SPO\!+ val SPO\!+ \\
\midrule
 & 600 & 0.005909 (500) & 0.005809 (130) & \textbf{0.005497 (490)} &
0.006110 (110) & 0.005610 (500) & FY val FY \\
\midrule
 & 1200 & 0.006085 (500) & -- & -- & 0.006093 (130) &
\textbf{0.005697 (500)} & SPO\!+ val SPO\!+ \\
\midrule
Large unseen & 50 & \textbf{0.005098 (500)} & 0.005107 (50) &
0.005411 (440) & 0.005127 (100) & 0.005220 (480) & 2stage val MSE \\
\midrule
 & 200 & 0.005127 (500) & \textbf{0.005125 (240)} & 0.005133 (500) &
0.005126 (130) & 0.005135 (500) & FY val gap \\
\midrule
 & 600 & \textbf{0.005094 (500)} & 0.005114 (130) & 0.005140 (490) &
0.005116 (110) & 0.005136 (500) & 2stage val MSE \\
\midrule
 & 1200 & 0.005124 (500) & -- & -- & 0.005129 (130) &
\textbf{0.005109 (500)} & SPO\!+ val SPO\!+ \\
\bottomrule
\end{tabular}}
\end{table}

\paragraph{How to read Table~\ref{tab:main-pk-step1bc}.}
This is the main direct comparison table. Bold marks the lowest mean normalized
gap within each test-set/train-size row, and the number in parentheses is the
selected checkpoint epoch. The missing FY entries at \(n=1200\) reflect the
strict Step1b validation2000 FY run not being available in the local combined
archive when this table was generated. The table makes the main pattern compact:
\spoplus{} selected by validation \spoplus{} loss is strongest on heldout400 for
most available sizes, while the large unseen test is much closer and often
favors 2stage or a near-tie.
...
This is a per-graph distribution plot, not a mean-performance plot. The box is
the middle 50\% of graphs, the whiskers show the 5th--95th percentile range, and
the diamond marks the mean. Because roughly half of the graphs have zero or
near-zero gap, the boxes are compressed near zero; the useful signal is mainly
in the upper whisker and mean diamond, which show whether a method makes fewer
large mistakes on harder graphs.
...
\midrule
50 & \textbf{SPO+} & \textbf{val SPO+} & \textbf{480} & \textbf{9.1155} & \textbf{4.1380} & \textbf{0.7401} & \textbf{0.005442} & \textbf{0.1079} & \textbf{[0.0347, 0.1960]} \\
\midrule
200 & 2stage & val MSE & 500 & 9.8848 & 5.1038 & 0.8451 & 0.006095 & -- & -- \\
\midrule
200 & SPO+ & val gap & 130 & 7.0334 & 3.5855 & 0.8185 & 0.005891 & 0.0266 & [-0.0076, 0.0830] \\
\midrule
200 & \textbf{SPO+} & \textbf{val SPO+} & \textbf{500} & \textbf{9.0462} & \textbf{4.4396} & \textbf{0.7706} & \textbf{0.005610} & \textbf{0.0745} & \textbf{[0.0170, 0.1527]} \\
\midrule
600 & 2stage & val MSE & 500 & 9.8899 & 5.0963 & 0.8223 & 0.005909 & -- & -- \\
\midrule
600 & SPO+ & val gap & 110 & 6.8532 & 3.5499 & 0.8480 & 0.006110 & -0.0258 & [-0.0734, 0.0000] \\
\midrule
600 & \textbf{SPO+} & \textbf{val SPO+} & \textbf{500} & \textbf{9.1956} & \textbf{4.5117} & \textbf{0.7706} & \textbf{0.005610} & \textbf{0.0517} & \textbf{[0.0044, 0.1112]} \\
\midrule
...
\midrule
1200 & \textbf{SPO+} & \textbf{val SPO+} & \textbf{500} & \textbf{9.0009} & \textbf{4.4856} & \textbf{0.7846} & \textbf{0.005697} & \textbf{0.0594} & \textbf{[0.0033, 0.1312]} \\
\bottomrule
\end{tabular}}
\end{table}
...
first show the combined result figures in Section~5, then use the following
plots only as backup diagnostics if the discussion turns to why the aggregate
bars behave the way they do.
...
\paragraph{How to read the Step1b post-hoc diagnostic figures.}
...
\begin{enumerate}[label=\textbf{C\arabic*.}, leftmargin=2.2em]
    ...
    \item \textbf{On heldout400, SPO+ selected by validation SPO+ loss gives the
    clearest Step1c signal.}  It consistently improves over 2stage across all
    four Step1c train sizes. Compared with the strict FY validation-loss
    checkpoint, SPO+ is better at \(n=50\) and \(n=200\), slightly worse at
    \(n=600\), and FY \(n=1200\) is not yet available in the strict archive.
...
that it can serve as an appendix reference. In the main presentation, it is
enough to say that the complete artifact roadmap is included here and then
return to the conclusions above.
...
\midrule
Step1c train-size 50 & \artifactpath{surrogate_experiment_results/Step1c/remote_results/formal_spoplus_ablation_val2000/train_size=50/metrics/unseen10000_summary.csv} & Large-unseen SPO+ summary. \\
\midrule
...
\midrule
Step1c train-size 200 & \artifactpath{surrogate_experiment_results/Step1c/remote_results/formal_spoplus_ablation_val2000/train_size=200/metrics/unseen10000_summary.csv} & Large-unseen SPO+ summary. \\
\midrule
...
\midrule
Step1c train-size 600 & \artifactpath{surrogate_experiment_results/Step1c/remote_results/formal_spoplus_ablation_val2000/train_size=600/metrics/unseen10000_summary.csv} & Large-unseen SPO+ summary. \\
\midrule
...
\midrule
Step1c train-size 1200 & \artifactpath{surrogate_experiment_results/Step1c/remote_results/formal_spoplus_ablation_val2000/train_size=1200/metrics/unseen10000_summary.csv} & Large-unseen SPO+ summary. \\
\midrule
